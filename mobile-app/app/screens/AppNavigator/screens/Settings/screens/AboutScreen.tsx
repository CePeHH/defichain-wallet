  ...
  return (
    <ThemedScrollViewV2 style={tailwind("flex-1")}>
      <View style={tailwind("flex-1 items-center justify-center mt-12 mb-11")}>
        <AppIcon height={52} testID="app_logo" width={52} />
        ...
        {hasBetaFeatures && (
          ...
          >
            <ThemedTextV2
              style={tailwind("mt-1 mb-1 text-2xs font-bold-v2 uppercase")}
            >
      ...
      {MAIN_LINKS.map((link) => (
        <LinkItemRow {...link} key={link.testID} />
      ))}
  ...
  return (
    ...
    >
      <View style={tailwind("flex-col flex-1 mr-8")}>
        <ThemedTextV2 style={tailwind("font-semibold-v2 text-sm flex-wrap")}>
...
function AboutLinkItem({
  ...
  return (
    <ThemedTouchableListItem
      testID={link.testID}
      isLast={isLast}
      onPress={async () => await openURL(link.url)}
      key={link.testID}
    >
      <ThemedTextV2 style={tailwind("flex-1 font-normal-v2 text-sm")}>
